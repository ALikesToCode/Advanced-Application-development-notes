chapter-5: Asynchronous Operations and Lifecycle Management in Modern Web Development
Introduction
...
beforeCreate: Called synchronously after the instance has been initialized, but before data observation and event/watcher setup.
...
created: Called after the instance is created. At this point, data observation and event/watcher setup have been done. However, the $el property is not yet available.
...
beforeMount: Called right before the mounting begins, and the render function is about to be called for the first time.

mounted: Called after the instance has been mounted to the DOM. Now, the this.$el property is available.

beforeUpdate: Called when data changes, before the DOM is re-rendered.

updated: Called after the DOM has been updated with the changes.

beforeDestroy: Called right before the instance is destroyed. At this stage, the instance is still fully functional.

destroyed: Called after the Vue instance has been destroyed. All event listeners and watchers are removed.
